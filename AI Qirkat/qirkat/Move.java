...
class Move {
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.isVestigial()) {
            return Move.move(move0.col0(), move0.row0(), move1.col0(),
                    move1.row0(), Move.move(move1.col0(), move1.row0(),
                            move1.col1(), move1.row1()));
        }
        if (move0.jumpTail() == null) {
            return Move.move(move0.col0(), move0.row0(), move0.col1(),
                    move0.row1(), move1);
        } else {
            Move m = move0.jumpTail();
            while (m.jumpTail() != null) {
                m = m.jumpTail();
            }
            m = Move.move(m, move1);
            return move0;
     ...
     *  the left. */
    boolean isLeftMove() {
        if (toIndex() - fromIndex() == -1 && !isJump() && row0() == row1()) {
            return true;
        }
        return false;
    }
     ...
     *  first leg of the jump.  non-capturing move, same as row1(). */
    char jumpedRow() {
        if (!this.isJump() || row1() == row0()) {
            return row1();
        } else {
            if (row1()  > row0()) {
                return (char) (row0() + 1);
    ...
    private void toString(Formatter out) {
        Move temp = this;
        out.format("%1$c%2$c-%3$c%4$c", temp.col0(), temp.row0(),
                temp.col1(), temp.row1());
        while (temp.jumpTail() != null) {
            temp = temp.jumpTail();
            out.format("-%1$c%2$c", temp.col1(), temp.row1());
        }
    }
